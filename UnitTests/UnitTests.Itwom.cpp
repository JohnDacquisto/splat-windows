...
{
	double dbLoss = IrregularTerrainModelAreaCalculationDbLoss(3, 2.3, 20.0, 7.620, 13.0, 1, 2, 15.0, 0.005, 301.0, 1000.0, 10.0, 0.5, 0.0, 605.0, 5, 0, 0, 0.9, 0.5, 0.75);
...
TEST(ItwomTests, CalculateHorizonDistancesTests)
{
	...
	EXPECT_DOUBLE_EQ(1005.8039999999999, prop.horizonDistance[0]);
	EXPECT_DOUBLE_EQ(1005.8039999999999, prop.horizonDistance[1]);
	EXPECT_DOUBLE_EQ(-0.012364951544002996, prop.grazingAngle[0]);
	EXPECT_DOUBLE_EQ(0.012252170678616918, prop.grazingAngle[1]);
}
...
TEST(ItwomTests, CalculateHorizonDistancesV2Tests)
{
	...
	EXPECT_DOUBLE_EQ(1005.8039999999999, prop.horizonDistance[0]);
	EXPECT_DOUBLE_EQ(1005.8039999999999, prop.horizonDistance[1]);
	EXPECT_DOUBLE_EQ(-0.012364330015386983, prop.grazingAngle[0]);
	EXPECT_DOUBLE_EQ(0.012251549150000905, prop.grazingAngle[1]);
	EXPECT_DOUBLE_EQ(83.816999999999993, prop.transmitterIntervalWidth);
	EXPECT_DOUBLE_EQ(20.0, prop.transmitterTotalHeight);
	EXPECT_DOUBLE_EQ(7.6200000000000001, prop.receiverTotalHeight);
	EXPECT_DOUBLE_EQ(0.0, prop.horizonHeight);
	EXPECT_DOUBLE_EQ(0.0, prop.hht);
	EXPECT_DOUBLE_EQ(0.0, prop.hhr);
	EXPECT_EQ(11, prop.horizonHeightIndex);
	EXPECT_EQ(1, prop.lineOfSight);
}
...
TEST(ItwomTests, CalculateTerrainInterdecileRangeDeltaV2Tests)
{
	...
	double rangeDelta = CalculateTerrainInterdecileRangeDeltaV2(pfl, xl[0], xl[1], propa);

	EXPECT_DOUBLE_EQ(0.0, rangeDelta);
}
...
TEST(ItwomTests, PrepareForPointToPointModeAnalysisTests)
{
	...
	PrepareForPointToPointModeAnalysis(pfl, propv.radioClimate, propv.variabilityMode, prop, propa, propv);

	EXPECT_DOUBLE_EQ(0.0, prop.referenceAttenuation);
	EXPECT_DOUBLE_EQ(0.0, prop.interdecileElevationRangeBetweenPoints);
	EXPECT_DOUBLE_EQ(20.0, prop.antennaEffectiveHeight[0]);
	EXPECT_DOUBLE_EQ(7.6200000000000001, prop.antennaEffectiveHeight[1]);
	EXPECT_DOUBLE_EQ(18887.260205892653, prop.horizonDistance[0]);
	EXPECT_DOUBLE_EQ(11658.198654613056, prop.horizonDistance[1]);
	EXPECT_DOUBLE_EQ(-0.0021178296674030234, prop.grazingAngle[0]);
	EXPECT_DOUBLE_EQ(-0.0013072345438177669, prop.grazingAngle[1]);
	EXPECT_DOUBLE_EQ(30545.458860505707, propa.dlsa);
	EXPECT_DOUBLE_EQ(-34.56469921753316, propa.ael);
	EXPECT_DOUBLE_EQ(0.0017020826384304896, propa.ak1);
	EXPECT_DOUBLE_EQ(0.0, propa.ak2);
	EXPECT_DOUBLE_EQ(-26.147761267992017, propa.aed);
	EXPECT_DOUBLE_EQ(0.0014265281611519054, propa.emd);
	EXPECT_DOUBLE_EQ(18887.260205892653, propa.dls[0]);
	EXPECT_DOUBLE_EQ(11658.198654613056, propa.dls[1]);
	EXPECT_DOUBLE_EQ(30545.458860505707, propa.dla);
	EXPECT_DOUBLE_EQ(-0.0034250642112207898, propa.tha);
	EXPECT_EQ(4, propv.recomputeParameters);
}
...
TEST(ItwomTests, PrepareForPointToPointModeAnalysisV2Tests)
{
	...
	PrepareForPointToPointModeAnalysisV2(pfl, propv.radioClimate, propv.variabilityMode, prop, propa, propv);

	EXPECT_DOUBLE_EQ(22.0, prop.referenceAttenuation);
	EXPECT_DOUBLE_EQ(20.0, prop.antennaHeightAndPathElevation[0]);
	EXPECT_DOUBLE_EQ(7.6200000000000001, prop.antennaHeightAndPathElevation[1]);
	EXPECT_DOUBLE_EQ(0.0, prop.interdecileElevationRangeBetweenPoints);
	EXPECT_DOUBLE_EQ(20.0, prop.antennaEffectiveHeight[0]);
	EXPECT_DOUBLE_EQ(7.6200000000000001, prop.antennaEffectiveHeight[1]);
	EXPECT_DOUBLE_EQ(1005.8039999999999, prop.horizonDistance[0]);
	...
	EXPECT_DOUBLE_EQ(0.0, prop.hhr);
	EXPECT_DOUBLE_EQ(20.0, prop.transmitterGroundHeight);
	EXPECT_DOUBLE_EQ(0.0, prop.tsgh);
	EXPECT_DOUBLE_EQ(0.0, prop.thera);
	EXPECT_DOUBLE_EQ(0.0, prop.thenr);
	EXPECT_DOUBLE_EQ(30545.458860505707, propa.dlsa);
	EXPECT_DOUBLE_EQ(2000000.0, propa.dx);
	EXPECT_DOUBLE_EQ(18887.260205892653, propa.dls[0]);
	EXPECT_DOUBLE_EQ(11658.198654613056, propa.dls[1]);
	EXPECT_DOUBLE_EQ(2011.6079999999997, propa.dla);
	EXPECT_DOUBLE_EQ(-0.00011278086538607859, propa.tha);
	EXPECT_EQ(11, prop.horizonHeightIndex);
	EXPECT_EQ(4, propv.recomputeParameters);
}
